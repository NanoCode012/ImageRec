...
###
def build_cnn( cnn ):
    if cnn.lower() == 'densenet201':
        return DenseNet201(include_top=False, weights='imagenet', pooling='avg')
    try:
        return CNN(cnn)
    except:
        msg = "Can not build: {}\n".format( cnn )
...
class Classifier:
    def __init__( self, cnn, clf=None, aug=None, C=1.0 ):
        self.fe = cnn
        if isinstance(self.fe,  type(DenseNet201())):
            self.preprocess_fn = densenet_preprocess_input
        if( clf is None ):
    ...
    def fit( self, images, y ):
        ...
        images = [self.preprocess_fn(img) for img in images] 
